...
export default function InfoAge() {
  ...
  const ages = Array.from({ length: 100 }, (_, i) => i.toString());
  ...
  return (
    ...
    >
      ...
      style={{
        ...
        <WheelPickerExpo
                height={300}
                width={150}
                initialSelectedIndex={ages.indexOf(userAge)}
                items={ages.map(age => ({ label: age, value: age }))}
                onChange={({ item }) => setUserAge(item.label)}
                 />
    ...
    <TouchableOpacity
      onPress={()=>{
        const age = userAge
        setInfoData({...infoData, age})
        router.push('/create-workout/info-goal')
      }}
      style={{
        bottom:-100,
        padding:15,
